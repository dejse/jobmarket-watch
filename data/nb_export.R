...
for (link in link_list) {
    ...
    tryCatch(
      {
        ...
        if (resp_status(response) == 200) {
          ...
          print(glue(
            "Attempt {attempt}: Status code {resp_status(response)} from {link}"
          ))
        }
      },
      error = function(e) {
        print(glue(
          "Attempt {attempt}: Request failed for {link} - {conditionprint(e)}"
        ))
      }
    )
  }
  ...
  if (is.null(response) || resp_status(response) != 200) {
    print(glue(
      "Failed to retrieve data from {link} after {max_retries} attempts"
    ))
    next
  }
